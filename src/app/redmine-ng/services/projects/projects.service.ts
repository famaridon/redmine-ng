...
import {Membership, Paginable, Project} from '../beans';
...
export class ProjectsService extends AbstractRedmineService<Project> {
    ...
    public findMembershipsByIdAndPermission(project_id: number, permissions: string): Observable<Paginable<Membership>> {
        return this.get(`/projects/${project_id}/memberships/${permissions}`).map((data: any) => {
            return new Paginable<Membership>(data, (json) => { return new Membership(json);});
        });
    }
